<template>
  <view class="uploade">
    <view class="uploade-button">
      <view class="title"> 请上传您的房源照片（至少一张） </view>
      <scroll-view class="uploade-scroll" :scroll-x="true">
        <view
          class="uploade"
          @click="uploadHead"
          v-if="formData.files?.length < 10"
        >
    ...
    <view class="uploade-col" @click="provinceShow = true">
      <view class="uploade-title"> 房源所在地址 </view>
    ...
    <view class="uploade-col">
      <view class="uploade-title"> 可接待房客数 </view>
      <view class="col-select">
        <view class="tips">
        ...
        <uni-number-box
          :min="1"
          v-model="formData.houseAmount"
          @change="handleHoseAmountChange"
        ></uni-number-box>
      </view>
    </view>
    <view class="uploade-col" @click="handleSetTime">
      <view class="uploade-title"> 开放换宿时间 </view>
      <view class="col-select">
        <view class="tips">
    ...
    <view class="uploade-col">
      <view class="uploade-title"> 联系方式 </view>
      <view class="col-select">
        <uni-easyinput
          :inputBorder="false"
          maxlength="30"
    ...
    <view class="uploade-col-textarea">
      <view class="uploade-title"> 对房客说的话 </view>
      <view class="col-input">
        <uni-easyinput
          ...
          :autoHeight="false"
          placeholder="请输入内容"
          v-model="formData.describle"
        ></uni-easyinput>
        <view class="sum"> {{ formData.describle.length ?? 0 }} / 300 </view>
      </view>
    </view>
  </view>
  <!-- 按钮 -->
...
onLoad((option) => {
  ...
  if (option.edit) {
    ...
    if (editHouse.cityCode) {
      ...
      form.address =
        houseArea[0].countryName +
        "-" +
        houseArea[1].regionName +
        "-" +
        houseArea[2].cityName;
    }
    //含区县
  ...
  uni.chooseImage({
    ...
    success: async (e) => {
      ...
      e.tempFilePaths.forEach((item, i) => {
        //判断文件后缀名
        let imgName =
          e.tempFilePaths[i].split(".")[
            e.tempFilePaths[i].split(".").length - 1
          ];
...
const handleUploadHouse = async (status) => {
  ...
  let data = {
    ...
    ...formData.value,
    houseImgPath: formData.value.files,
  };
